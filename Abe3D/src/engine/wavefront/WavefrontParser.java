...
public class WavefrontParser {
	...
	public static List<Obj> load(String resource, double scaleFactor) throws Exception {
		...
		BufferedReader reader = new BufferedReader(new InputStreamReader(WavefrontParser.class.getResourceAsStream(resource)));
	...
	private static void extractVertex(String line, List<Vec4> vertices, double scaleFactor) {
		...
		String[] v = line.split("\\ ");
		Vec4 vertex = new Vec4(Double.parseDouble(v[0]), Double.parseDouble(v[1]), Double.parseDouble(v[2]), 1);
	...
	private static void extractVertexNormal(String line, List<Vec4> normals) {
		line = line.substring(3).trim();
		String[] v = line.split("\\ ");
		Vec4 normal = new Vec4(Double.parseDouble(v[0]), Double.parseDouble(v[1]), Double.parseDouble(v[2]), 1);
		normals.add(normal);
	}
