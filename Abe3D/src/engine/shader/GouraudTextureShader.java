...
public class GouraudTextureShader extends Shader {
	...
	public void processPixel(Renderer renderer, int xMin, int xMax, int x, int y, double[] vars) {
		...
		color[0] = 255;
		color[1] = (int)MathUtils.clamp(color[1] * colorp1 * scale, 0, 255);
		color[2] = (int)MathUtils.clamp(color[2] * colorp2 * scale, 0, 255);
		color[3] = (int)MathUtils.clamp(color[3] * colorp3 * scale, 0, 255);
